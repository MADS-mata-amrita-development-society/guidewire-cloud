...
export function DriversListPage() {
  ...
  useEffect(() => {
    ...
    fetchDrivers(profile.company_id).then(({ data }) => {
      setDrivers(data || []);
      setLoading(false);
    });
  }, [profile]);
    ...
    <div className="page-content">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{drivers.length} in your fleet</p>
        </div>
      </div>

      <div style={{ marginBottom: 'var(--space-3)', maxWidth: '320px' }}>
        <Input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} icon={<MagnifyingGlass size={16} />} />
      </div>

      <div className="table-container">
        <table className="table">
          <thead>
            <tr><th>Driver</th><th>Zone</th><th>Tier</th><th>Balance</th></tr>
          </thead>
          <tbody>
            {filtered.map((d: any) => (
              <tr key={d.id}>
